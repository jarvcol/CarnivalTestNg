...
public class CarnivalHomePage extends BasePage {
	...
	@FindBy(xpath = "//button[@class='cdc-filter-button' and @aria-label='Mexico ']")
	private WebElement mexicoFilterButton;
	...
	@FindBy(css = ".vrg-search-unit .vrgf-learn-more")
	...
	public boolean verifyPricingFilter() {
		...
		for (WebElement currentPrice : listOfPrices) {
			if (Integer.parseInt(currentPrice.getText().replaceAll("[^\\d]", "")) < minVal
					&& Integer.parseInt(currentPrice.getText().replaceAll("[^\\d]", "")) > maxVal)
				return false;
		}
		return true;
	}
	...
	public boolean verifySortResults() {
		if (sortIcon.getAttribute("alt").equals("High to low")) {
			...
			if (Integer.parseInt(listOfPrices.get(0).getText().replaceAll("[^\\d]", "")) >= Integer
					.parseInt(listOfPrices.get(listOfPrices.size() - 1).getText().replaceAll("[^\\d]", "")))
				return true;
		}
	...
	public ItineraryPage getIntoFirstResult() {
		getWait().until(ExpectedConditions.refreshed(ExpectedConditions.visibilityOfAllElements(learnMoreButtons)));
